...
def print_report(pure: Dict[str, object], pure_bytes: bytes) -> None:
    ...
    if bad:
        ...
        # --- ARTIFACT EXPORT (production): qg_screening_plot.png ---
        try:
            import os
            from artifact_screening import write_qg_screening_plot
            ...
            # eps0: try several canonical locations
            _eps0 = None
            _eps0c = pure.get("eps0_closure") or pure.get("eps0") or {}
            if isinstance(_eps0c, dict):
                for path in (
                    ("canonical","eps0"),
                    ("best","eps0"),
                ):
                    sub = _eps0c.get(path[0])
                    if isinstance(sub, dict):
                        _eps0 = _to_float(sub.get(path[1]))
                        if _eps0 is not None:
                            break
                if _eps0 is None:
                    _eps0 = _to_float(_eps0c.get("eps0"))
            if _eps0 is None:
                _eps0 = _to_float(pure.get("eps0")) or _to_float(pure.get("eps0_canon"))

            # g_eff: prefer direct, else compute from R_inf
            _rg = pure.get("rg_fit") or pure.get("rg") or {}
            _geff = None
            if isinstance(_rg, dict):
                _geff = _to_float(_rg.get("g_eff")) or _to_float(_rg.get("geff"))
                if _geff is None:
                    _Rinf = _to_float(_rg.get("R_inf")) or _to_float(_rg.get("Rinf"))
                    if _Rinf is not None:
                        _geff = (_Rinf - 1.0) / 12.0

            if _eps0 is None or _geff is None:
                raise RuntimeError(f"missing eps0/geff for artifact: eps0={_eps0} geff={_geff}")

            _alphaA = {k: alpha_eff_piecewise(v, _eps0, _geff) for k,v in EPS_POINTS.items()}
            _alphaB = {k: alpha_eff_smooth(v, _eps0, _geff) for k,v in EPS_POINTS.items()}

            _art_dir = os.path.join(os.path.dirname(__file__), "_artifacts")
            os.makedirs(_art_dir, exist_ok=True)
            _art_path = os.path.join(_art_dir, "qg_screening_plot.png")
            write_qg_screening_plot(_alphaA, _alphaB, _art_path, eps0=_eps0, g_eff=_geff, max_rel_delta=_to_float(pure.get("screening",{}).get("max_rel_delta")))
            print(f"ARTIFACT written: {_art_path}")
        except Exception as _e:
            print(f"ARTIFACT FAILED: qg_screening_plot.png ({_e})")
