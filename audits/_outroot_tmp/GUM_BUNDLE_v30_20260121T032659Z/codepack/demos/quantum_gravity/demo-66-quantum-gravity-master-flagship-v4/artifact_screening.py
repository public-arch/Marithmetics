...
def write_qg_screening_plot(alphaA: dict, alphaB: dict, out_path: str) -> None:
    ...
    # Legend (top-right): blue = Witness A, red = Witness B
    lx, ly = w - 240, y0 + 10
    # box
    for x in range(lx, lx+220):
        put(x, ly, (0,0,0)); put(x, ly+46, (0,0,0))
    for y in range(ly, ly+47):
        put(lx, y, (0,0,0)); put(lx+219, y, (0,0,0))
    # markers
    for dx in range(-4,5):
        for dy in range(-4,5):
            if dx*dx+dy*dy <= 10:
                put(lx+18+dx, ly+16+dy, (0,90,200))   # A
                put(lx+18+dx, ly+34+dy, (200,30,30))  # B
    # pseudo-label ticks (no font): two horizontal lines to indicate two series
    for x in range(lx+32, lx+210):
        put(x, ly+16, (0,90,200))
        put(x, ly+34, (200,30,30))

    # X-axis tick marks under each body point
    for x in xs:
        for dy in range(0, 8):
            put(x, y1+dy, (0,0,0))
