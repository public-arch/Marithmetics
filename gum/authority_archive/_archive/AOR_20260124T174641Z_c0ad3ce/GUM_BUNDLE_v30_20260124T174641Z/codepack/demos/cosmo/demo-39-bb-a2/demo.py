    ...
    for h in h_vals:
        for M in M_vals:
            ...
            records["phi_hat_grid"].append({
                ...
                "float_decimal_ok": float_decimal_err <= 1e-12,
                "roundtrip_tol": tol,
                "repr_b7": b7,
                "rt_err_b7": abs(rt7 - ph_f),
                "repr_b10": b10,
                "rt_err_b10": abs(rt10 - ph_f),
                "repr_b16": b16,
                "rt_err_b16": abs(rt16 - ph_f),
            })
...
def build_markdown_report(result: Dict[str, object]) -> str:
    ...
    # Cross-base invariance snapshot (report-friendly)
    cb = result["records"].get("a2_cross_base_repr", {})
    if cb:
        lines.append("## Cross-base invariance")
        lines.append("")
        tol = 1e-15
        max_b7  = max(abs(v.get("rt_err_b7", 0.0))  for v in cb.values())
        max_b10 = max(abs(v.get("rt_err_b10", 0.0)) for v in cb.values())
        max_b16 = max(abs(v.get("rt_err_b16", 0.0)) for v in cb.values())
        lines.append(f"Max roundtrip error across all A2 constants (tol={tol:.0e}):")
        lines.append(f"- b7:  {max_b7:.2e}")
        lines.append(f"- b10: {max_b10:.2e}")
        lines.append(f"- b16: {max_b16:.2e}")
        lines.append("")
        keys = ["alpha", "alpha_s", "sin2", "H0", "Om_tot", "As", "ns", "tau", "ell1"]
        lines.append("| const | base10 | b7 | b16 | max|Δ| |")
        lines.append("|---|---:|---|---|---:|")
        for k in keys:
            if k not in cb:
                continue
            v = cb[k]
            max_err = max(abs(v.get("rt_err_b7", 0.0)), abs(v.get("rt_err_b10", 0.0)), abs(v.get("rt_err_b16", 0.0)))
            lines.append(f"| `{k}` | `{v.get('val', None)}` | `{v.get('b7', '')}` | `{v.get('b16', '')}` | `{max_err:.2e}` |")
        lines.append("")
    lines.append("")
    return "\n".join(lines)
...
def run_archive(mode: str = "fast") -> Dict[str, object]:
    ...
    result["passes"]["a2_numeric_invariant_under_repr"] = result["passes"]["a2_constants_roundtrip_parse_ok"]
...
def pretty_print(result: Dict[str, object], no_color: bool = False) -> None:
    ...
    # (A) φ̂(h,M) grid: representation↔value roundtrip checks
    grid = ros["records"]["phi_hat_grid"]
    max_rt_b7  = max(abs(r["rt_err_b7"])  for r in grid) if grid else 0.0
    max_rt_b10 = max(abs(r["rt_err_b10"]) for r in grid) if grid else 0.0
    max_rt_b16 = max(abs(r["rt_err_b16"]) for r in grid) if grid else 0.0
    p.kv("phi_hat roundtrip max |Δ| (b7/b10/b16)", f"{max_rt_b7:.2e} / {max_rt_b10:.2e} / {max_rt_b16:.2e}")

    # (B) A2 constants: explicit cross-base roundtrip equality
    cb = result["records"]["a2_cross_base_repr"]
    max_c_b7  = max(abs(v["rt_err_b7"])  for v in cb.values()) if cb else 0.0
    max_c_b10 = max(abs(v["rt_err_b10"]) for v in cb.values()) if cb else 0.0
    max_c_b16 = max(abs(v["rt_err_b16"]) for v in cb.values()) if cb else 0.0
    p.kv("A2 constants roundtrip max |Δ| (b7/b10/b16)", f"{max_c_b7:.2e} / {max_c_b10:.2e} / {max_c_b16:.2e}")
    p.kv("A2 constants roundtrip (all)", "PASS" if result["passes"]["a2_constants_roundtrip_parse_ok"] else "FAIL")

    keys = ["alpha", "alpha_s", "sin2", "H0", "Om_tot", "As", "ns", "tau", "ell1"]
    rows = [["const", "value (base10)", "repr b7", "repr b16", "max|Δ|"]] 
    for k in keys:
        if k not in cb:
            continue
        v = cb[k]
        max_err = max(abs(v["rt_err_b7"]), abs(v["rt_err_b10"]), abs(v["rt_err_b16"]))
        rows.append([k, _fmt_float(v["val"], sig=18), v["b7"], v["b16"], f"{max_err:.2e}"])
    p.table(rows)

    if result["passes"]["rosetta_all_pass"] and result["passes"]["a2_constants_roundtrip_parse_ok"]:
        p.ok("CLOSED: base representations (b7/b10/b16) roundtrip to the same numeric values (tol=1e-15).")
    else:
