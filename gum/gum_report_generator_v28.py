...
# ---------------------------------------------------------------------------
# DEMO-33 v10 runner (executes SM demo and loads raw + dressed outputs)
# ---------------------------------------------------------------------------

def run_demo33_v10(report_dir: str, want_overlay: bool = True) -> Dict[str, Any]:
    """
    Run sm/sm_standard_model_demo_v1.py (DEMO-33 v10) as a subprocess and load its JSON artifacts.

    The demo is pure-by-default:
      - sm_outputs_pure.json is produced without PDG inputs.
      - sm_outputs_overlay.json is evaluation-only (PDG overlay), if --overlay is run.

    Returns a dict:
      ok, out_dir, pure, pure_sha256, overlay, overlay_sha256, (or error diagnostics).
    """
    gum_dir = os.path.abspath(os.path.dirname(__file__))
    repo_root = os.path.abspath(os.path.join(gum_dir, ".."))
    sm_path = os.path.join(repo_root, "sm", "sm_standard_model_demo_v1.py")

    out_dir = os.path.join(report_dir, "DEMO33_v10")
    os.makedirs(out_dir, exist_ok=True)

    env = os.environ.copy()
    env["DEMO_OUT_DIR"] = out_dir
    env["PYTHONPATH"] = repo_root + os.pathsep + env.get("PYTHONPATH", "")

    def _run(args: List[str]) -> Tuple[int, str, str]:
        p = subprocess.run(
            [sys.executable, sm_path] + args,
            cwd=repo_root,
            env=env,
            capture_output=True,
            text=True,
    ...
    try:
        # Hard selftest gate first
        rc, out, err = _run(["--selftest"])
        if rc != 0:
            return {"ok": False, "stage": "selftest", "rc": rc, "stdout": out, "stderr": err, "out_dir": out_dir}

        # Pure run (produces sm_outputs_pure.json)
        rc, out, err = _run([])
        if rc != 0:
            return {"ok": False, "stage": "pure", "rc": rc, "stdout": out, "stderr": err, "out_dir": out_dir}

        pure_path = os.path.join(out_dir, "sm_outputs_pure.json")
        if not os.path.exists(pure_path):
            return {"ok": False, "stage": "pure", "error": "sm_outputs_pure.json not found", "out_dir": out_dir}

        with open(pure_path, "r", encoding="utf-8") as f:
            pure = json.load(f)
        pure_sha = sha256_of_file(pure_path)

        overlay = None
        overlay_sha = None
        overlay_path = os.path.join(out_dir, "sm_outputs_overlay.json")

        if want_overlay:
            rc2, out2, err2 = _run(["--overlay"])
            # Overlay is evaluation-only; do not hard-fail the report if it can't run.
            if rc2 == 0 and os.path.exists(overlay_path):
                with open(overlay_path, "r", encoding="utf-8") as f:
                    overlay = json.load(f)
                overlay_sha = sha256_of_file(overlay_path)

        return {
            "ok": True,
            "out_dir": out_dir,
            "pure": pure,
            "pure_sha256": pure_sha,
            "overlay": overlay,
...
def build_pdf_report(pdf_path: str,
    ...
    # Reproducibility note (repo link + first-principles demos)
    story.append(Spacer(1, 0.12 * inch))
    story.append(
        Paragraph(
            "<b>Reproducibility.</b> All results in this report are generated by first-principles Python demos "
            "in the public Marithmetics repository. A full run of the report, including referenced demos and assets, "
            "is reproducible from source at: "
            "<font color='blue'>https://github.com/public-arch/Marithmetics</font>",
            small,
        )
    )
    ...
    # ------------------------------------------------------------
    # 1.2 Standard Model outputs (DEMO-33 v10): raw vs dressed
    # ------------------------------------------------------------
    story.append(Spacer(1, 0.18 * inch))
    story.append(Paragraph("1.2 Standard Model outputs (DEMO-33 v10): raw vs dressed", h2))
    ...
    if isinstance(demo, dict) and demo.get("ok"):
        ...
        story.append(
            Paragraph(
                ...
                "in a pure artifact (no PDG inputs). Optional PDG comparisons, when present, are evaluation-only "
                "and are not used upstream.",
                body,
            )
        )
        story.append(Spacer(1, 0.06 * inch))
        story.append(Paragraph(f"DEMO-33 v10 artifact SHA-256 (pure): {demo.get('pure_sha256','NA')}", small))
        if demo.get("overlay_sha256"):
            story.append(Paragraph(f"DEMO-33 v10 artifact SHA-256 (overlay): {demo.get('overlay_sha256')}", small))

        ov = demo.get("overlay") or {}
        ov_sm28 = (ov.get("overlay", {}) or {}).get("sm28", {})
        if isinstance(ov_sm28, dict) and "closed" in ov_sm28:
            flag = "PASS" if ov_sm28.get("closed") else "FAIL"
        ...
        ew_rows = [
            ...
            ["sin2W", fmt_val(pure.get("sin2thetaW", pure.get("sin2W"))), fmt_val(pred.get("sin2thetaW_dressed", pred.get("sin2W"))), fmt_val(SM_REF.get("sin2W"))],
            ["alpha_s", fmt_val(pure.get("alpha_s_MZ", pure.get("alpha_s"))), fmt_val(pred.get("alpha_s_MZ", pred.get("alpha_s"))), fmt_val(SM_REF.get("alpha_s"))],        
        ...
        if names:
            ...
            story.append(Paragraph("SM-28 closure vector (raw vs dressed)", small))

            sm28_rows = [["Name", "Raw", "Dressed", "Tier"]]
            for name in names:
                rr = raw_map.get(name, {})
                pr = pred_map.get(name, {})
                tier = pr.get("tier") or rr.get("tier") or ""
                sm28_rows.append(
                    [name, fmt_val(rr.get("value"), sci=True, digits=6), fmt_val(pr.get("value"), sci=True, digits=6), tier]
                )

            sm28_table = Table(sm28_rows, hAlign="LEFT", colWidths=[180, 95, 95, 70], repeatRows=1)
            sm28_table.setStyle(
                TableStyle(
                    [
                        ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
        ...
        story.append(
            Paragraph(
                ...
                "sm/sm_standard_model_demo_v1.py or could not locate sm_outputs_pure.json in the run directory.",
                body_italic,
            )
        )
    ...
    story.append(
        Paragraph(
            ...
            "(see Table 1.2). These are treated as falsifiable program outputs rather than fitted parameters: "
    ...
    story.append(PageBreak())
    story.append(Paragraph("8. Reproducibility and source links", h1))

    story.append(
        Paragraph(
            "All figures and tables in this report are generated by first-principles Python scripts in the "
            "Marithmetics repository. The definitive source location is:",
            body,
        )
    )
    story.append(
        Paragraph(
            "<font face='Courier'>https://github.com/public-arch/Marithmetics</font>",
            body,
        )
    )

    story.append(Spacer(1, 0.15 * inch))
    story.append(Paragraph("Primary entry points used in this report:", h2))
    ...
    story.append(
        Paragraph(
            "<font face='Courier'>gum/gum_v1.py</font>  — unified GUM driver (substrate → math → SM → cosmology)",
            body,
        )
    )
    story.append(
        Paragraph(
            "<font face='Courier'>sm/sm_math_model_demo_v1.py</font>  — SM-MATH-9 (math constants)",
            body,
        )
    )
    story.append(
        Paragraph(
            "<font face='Courier'>sm/sm_standard_model_demo_v1.py</font>  — DEMO-33 (Standard Model layer)",
            body,
        )
    )
    story.append(
        Paragraph(
            "<font face='Courier'>cosmo/bb_grand_emergence_masterpiece_runner_v1.py</font>  — BB-36 (structural cosmology)",
            body,
        )
    )
...
def main() -> None:
    ...
    payload["demo33_v10"] = run_demo33_v10(report_dir, want_overlay=True)
