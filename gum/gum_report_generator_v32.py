...
def build_bridge_section(styles: Dict[str, ParagraphStyle]) -> List[Any]:
    ...
    story.append(P(
        "A common critique of integer→physics programs is that the bridge from discrete structure to continuum closure can feel abrupt. This launch report is written as an external-facing Authority-of-Record: it makes the bridge explicit and auditable. The core claim is not parameter tuning but constrained emergence: constants behave like structural eigenvalues of the discrete kernel. Every numeric claim is tied to rerunnable demos plus a cryptographic evidence ledger (bundle hash + per-file hashes). "
            ...
            if structured_rows:
                ...
                for row in picked:
                    const_rows.append([
                        ...
                        str(row.get("value", "")),
                        str(row.get("units", "")),
                        str((row.get("source") or row.get("source_path") or row.get("source_file") or row.get("source_sha256") or "-"))[:120],
                    ])
