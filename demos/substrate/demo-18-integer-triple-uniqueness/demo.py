...
Claim:
  The primary integer triple (137, 107, 103) is the unique output of
  five number-theoretic gates derived from Phi-structure.  The triple
  is a necessity, not a design choice.
...
The mod-8 branch is derived from TWO ingredients tied to Phi-structure:
  (1) Phi-parity via Legendre(2|q): alpha:+1, su2:-1, pc2:+1
      (q is largest odd prime factor of w-1)
  (2) Fejer/MST minimal envelope -> 2-adic branch v2(w-1): alpha:3, su2:1, pc2:1

Wheel orientation for PC2 via Legendre(5|q) = -1 excludes 83 and selects 103.
...
def emj(flag): return "PASS" if flag else "FAIL"
...
def summarize(channel, survivors, loose_parity=False, loose_v2=False, loose_5=False, use_emoji=True):
    ...
    print("    C4'   period extremality: {}  (q={}, q%4=1, q>sqrt(w))".format(
        emj(best["q_mod4_1"] and best["q_gt_sqrt"]), best["q"]))
    if loose_parity:
        print("    C4''  Phi-parity (Legendre(2|q)): [SKIPPED by --loose-parity]")
    else:
        print("    C4''  Phi-parity (Legendre(2|q)): {}".format(emj(best["leg2_ok"])))
    if loose_v2:
        print("    C2''  2-adic branch v2(w-1):     [SKIPPED by --loose-v2]")
    else:
        print("    C2''  2-adic branch v2(w-1):     {}".format(emj(best["v2_ok"])))
    need5 = WHEEL_ORIENTATION_5[channel]
    if need5 is None:
        ...
        if loose_5:
            ...
            print("    C5''  wheel orientation (5):      {}".format(emj(best["leg5_ok"])))
    print("    C6'   simplicity (prime):         {}".format(emj(best["prime"])))
    return w_best

def main():
    ap = argparse.ArgumentParser(description="DEMO-18 -- Integer Triple Uniqueness")
    ...
    print("=====================================================================")
    print("DEMO-18 -- Integer Triple Uniqueness")
    print("=====================================================================")
